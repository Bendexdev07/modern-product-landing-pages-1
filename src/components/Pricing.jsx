        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-6">
          {plans.map((plan, key) => {
            return (
              <div
                key={key}
                className={`relative bg-slate-900/50 backdrop-blur-sm border rounded-xl sm:rounded-2xl p-6 sm:p-8  transition-all overflow-visible group flex flex-col h-full ${
                  plan.mostPopular
                    ? "border-blue-500 shadow-2xl shadow-500/20 lg:scale-105"
                    : "border-slate-800 hover:slate-700"
                }`}
              >
                ...
                {plan.mostPopular && (
                  <div className="absolute -top-2 sm:-top-3 left-1/2 transform -translate-x-1/2 z-10">
                    <div className="flex items-center space-x-1 px-3 sm:px-4 py-1 sm:py-1.5 bg-gradient-to-b from-blue-500 to-cyan-500 rounded-full text-xs sm:text-sm font-semibold shadow-lg">
                      <Star className="w-3 h-3 sm:w-s h-3 fill-white " />
                ...
                <div className="text-center mb-6 sm:mb-8">
                  ...
                  <div className="flex items-baseline justify-center">
                    ...
                    <span className="grey-400 ml-1 sm:ml-2 text-sm sm:text-base">
                      /month
                    </span>
                  </div>
                </div>
                  ...
                  {plan.features.map((feature, featurekey) => (
                    ...
                    >
                      <div className="flex-shrink-0 w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-blue-500/20 flex items-center justify-center ">
                        <Check className="w-2.5 h-2.5 text-blue-400 sm:h-3 sm:w-4  " />
                      </div>
